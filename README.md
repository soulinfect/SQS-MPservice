...
## Soft Delete
Удаление записей реализовано через **мягкое удаление** с использованием флага `deleted` в базе данных.
- При получении сообщения с `action: "delete"` запись не удаляется физически из таблицы.
- Вместо этого полю `deleted` присваивается значение `true`.
